...
This guide walks you through setting up a Kubernetes **worker node** and joining it to the cluster.
...
Run this **only on the master node** (not worker):
...
On the master node, verify the worker node has joined:
